import { format } from 'date-fns'
import { CalendarIcon, EditIcon, ImagesIcon, TrashIcon } from 'lucide-react'

import { Button } from '#/components/ui/button'
import { ButtonGroup } from '#/components/ui/button-group'
import {
  Card,
  CardAction,
  CardContent,
  CardFooter,
  CardHeader,
  CardTitle,
} from '#/components/ui/card'
import { useAuthStore } from '#/stores/use-auth-store'
import type { Gallery } from '#/types/gallery'

const GalleryCard = ({
  data,
  onCardClick,
  onModifyButtonClick,
  onDeleteButtonClick,
}: {
  data: Gallery
  onCardClick: (data: Gallery) => void
  onModifyButtonClick: (data: Gallery) => void
  onDeleteButtonClick: (data: Gallery) => void
}) => {
  const user = useAuthStore((state) => state.user)
  const isAdmin = useAuthStore((state) => state.isAdmin)

  const thumbnail = data.images[0]?.url

  return (
    <Card
      className="overflow-hidden cursor-pointer hover:shadow-md transition-shadow gap-0"
      onClick={() => onCardClick(data)}
    >
      <div className="w-full h-48 bg-muted overflow-hidden">
        {thumbnail ? (
          <img
            src={thumbnail}
            alt={data.title}
            className="w-full h-full object-cover"
          />
        ) : (
          <div className="w-full h-full flex items-center justify-center text-muted-foreground">
            <ImagesIcon className="w-8 h-8" />
          </div>
        )}
      </div>

      <CardHeader className="pt-3 pb-1">
        <CardTitle className="text-base line-clamp-1">{data.title}</CardTitle>
      </CardHeader>

      <CardContent className="pb-2">
        <div className="flex flex-row items-center gap-3 text-xs text-muted-foreground">
          <div className="flex items-center gap-1">
            <CalendarIcon className="w-3.5 h-3.5" />
            <span>{format(data.date, 'MMM yyyy')}</span>
          </div>
          <div className="flex items-center gap-1">
            <ImagesIcon className="w-3.5 h-3.5" />
            <span>{data.images.length}</span>
          </div>
        </div>
        {data.description && (
          <p className="text-xs text-muted-foreground mt-1 line-clamp-2">
            {data.description}
          </p>
        )}
      </CardContent>

      {user && isAdmin && (
        <CardFooter onClick={(e) => e.stopPropagation()}>
          <CardAction>
            <ButtonGroup>
              <Button
                variant="outline"
                size="sm"
                onClick={() => onModifyButtonClick(data)}
              >
                <EditIcon className="w-3.5 h-3.5" />
                Edit
              </Button>
              <Button
                variant="destructive"
                size="sm"
                onClick={() => onDeleteButtonClick(data)}
              >
                <TrashIcon className="w-3.5 h-3.5" />
                Delete
              </Button>
            </ButtonGroup>
          </CardAction>
        </CardFooter>
      )}
    </Card>
  )
}

export { GalleryCard }
